...
class RR_API
{
  ...
  */
  ...
  public Vector <String> getVariables(String[] nameVec)
  {
	  StringBuilder sb = new StringBuilder();
	  for (int i=0; i < nameVec.length - 1; i++)
	  {
		  sb.append(nameVec[i]);
		  sb.append(", ");
	  }
	  if (nameVec.length > 0)
	  {
		  sb.append(nameVec[nameVec.length - 1]);
	  }
	  return getVariables(new String(sb));
  }
  /*
  ...
  */
  ...
  public Vector <String> getVariables(String names)
  {
    if (!connected) return null;
